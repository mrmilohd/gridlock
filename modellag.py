...
print("🚀 Initializing V8 Trifecta (Spatial Shockwaves Edition)...")
...
best_sub = pd.read_csv("submission_ultimate.csv")  # Your 87.94 submission
...
# ---------------------------------------------------------
# 3. Momentum & The Spatial Shockwaves (The Missing 5 Points)
# ---------------------------------------------------------
print(" -> Calculating Spatial Shockwaves (Traffic Bleed)...")
...
# Standard Neighborhood Momentum
...
# NEW: Neighborhood Shockwave (Max Traffic in prefix 15 mins ago)
df["neighborhood_shockwave_15m"] = df.groupby(["geohash_prefix", "time_decimal"])[
    "demand"
].transform("max")
...
df["lagged_shockwave"] = df["lagged_shockwave"].fillna(global_median)
for col in target_encode_cols:
    df[col] = df[col].fillna(global_median)

# NEW: Spatial-Categorical Cross
...
df = df.drop(
    ["timestamp", "minute_bin", "latitude", "longitude", "neighborhood_shockwave_15m"],
    axis=1,
)

# ---------------------------------------------------------
# 4. Prepare Multi-Seed Data
# ---------------------------------------------------------
train_and_pseudo = (
...
# ---------------------------------------------------------
# 5. Multi-Seed Training (The Grandmaster Technique)
# ---------------------------------------------------------
SEEDS = [42, 1337, 2026]
...
print("[3/3] Training CatBoost Forest...")
for seed in SEEDS:
...
submission.to_csv("submission_v8_shockwave.csv", index=False)
print("✅ Saved to submission_v8_shockwave.csv successfully!")
