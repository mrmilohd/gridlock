...
print("🚀 Initializing V9: The Log-Transformed Trifecta...")

# ---------------------------------------------------------
# 1. Load Data & Premium 88.15 Pseudo-Labels
# ---------------------------------------------------------
train = pd.read_csv("train.csv")
test = pd.read_csv("test.csv")
best_sub = pd.read_csv("submission_ultimate.csv")  # Your 88.15 submission
...
# ---------------------------------------------------------
# 4. Prepare Multi-Seed Data (WITH LOG TRANSFORMATION)
# ---------------------------------------------------------
train_and_pseudo = (
...
# 🔥 THE 94% SECRET: Apply log1p to the target variable 🔥
...
print("\n[1/3] Training LightGBM on Log-Scale...")
for seed in SEEDS:
    ...
    # Revert log scale for predictions using expm1
    lgb_preds += np.clip(np.expm1(model.predict(X_test)), 0, 1) / len(SEEDS)

print("[2/3] Training XGBoost on Log-Scale...")
for seed in SEEDS:
    ...
    xgb_preds += np.clip(np.expm1(model.predict(X_test)), 0, 1) / len(SEEDS)

print("[3/3] Training CatBoost on Log-Scale...")
for seed in SEEDS:
    ...
    model.fit(X_train_full, y_train_full)
    cat_preds += np.clip(np.expm1(model.predict(X_test)), 0, 1) / len(SEEDS)

# ---------------------------------------------------------
# 6. The Blended Exponentiation
# ---------------------------------------------------------
print("\n -> Blending Final Log-Transformed Predictions...")
...
submission.to_csv("submission_v9_log_transformed.csv", index=False)
print("✅ Saved to submission_v9_log_transformed.csv successfully!")
